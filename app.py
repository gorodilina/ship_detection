...
if mode == "🚢 Детекция кораблей":
    ...
    # ============ УЛУЧШЕННЫЙ ПОРОГ УВЕРЕННОСТИ ============
    st.sidebar.subheader("🎯 Порог уверенности детекции")
    
    conf_threshold = st.sidebar.slider(
        "Порог уверенности (confidence)", 
        min_value=0.01,     # Минимум 1%
        max_value=0.95,     # Максимум 95%
        value=0.50,         # ПО УМОЛЧАНИЮ 0.5 (50%)
        step=0.01,          # Шаг 1%
        format="%.2f",      # Формат отображения
        help="Выше порог = меньше детекций, но выше точность. Ниже порог = больше детекций, но больше ложных срабатываний."
    )
    
    # Индикатор текущего порога с цветовой шкалой
    if conf_threshold < 0.3:
        conf_color = "🟢"
        conf_desc = "Низкий (много детекций)"
    elif conf_threshold < 0.6:
        conf_color = "🟡"
        conf_desc = "Средний (сбалансированный)"
    else:
        conf_color = "🔴"
        conf_desc = "Высокий (только уверенные)"
    
    st.sidebar.markdown(f"{conf_color} **{conf_threshold:.0%}** - {conf_desc}")
    
    st.sidebar.markdown("---")
    
    # Остальные параметры
    st.sidebar.subheader("📐 Параметры тайлов")
...
""")
    ...
    if uploaded_file is not None:
        ...
        st.info(f"🔄 Обрабатываем изображение с помощью {model_name} (порог: {conf_threshold:.0%})...")
        ...
        def process_geotiff(geotiff_path, model, conf_threshold, tile_size, overlap):
            with rasterio.open(geotiff_path) as src:
                ...
                for i, det in enumerate(final_detections):
                    ...
                    conf = det['conf']
                    if conf >= 0.8:
                        color = 'lime'  # Высокая уверенность
                    elif conf >= 0.6:
                        color = 'yellow'  # Средняя
                    else:
                        color = 'orange'  # Низкая
        ...
        try:
            ...
            st.success(f"✅ Обнаружено кораблей: {len(ships)} (модель: {model_name}, порог: {conf_threshold:.0%})")
            
            # Статистика по уверенности
            if len(ships) > 0:
                confidences = [s['confidence'] for s in ships]
                ...
                cols = st.columns(4)
                with cols[0]:
                    st.metric("Всего кораблей", len(ships))
                with cols[1]:
                    st.metric("Средняя уверенность", f"{avg_conf:.0%}")
                with cols[2]:
                    st.metric("Мин. уверенность", f"{min_conf:.0%}")
                with cols[3]:
                    st.metric("Макс. уверенность", f"{max_conf:.0%}")
            ...
            for ship in ships:
                # Цвет маркера в зависимости от уверенности
...
🤖 Модель: {model_name}
            ...
            st.caption(f"💡 Наведите курсор для просмотра координат. (Модель: {model_name}, Порог: {conf_threshold:.0%})")
            
            map_data = st_folium(m, width=1400, height=700, returned_objects=[])
            
            st.subheader("📊 Детальная информация")
            
            # Сортируем корабли по уверенности
            ships_sorted = sorted(ships, key=lambda x: x['confidence'], reverse=True)
            
            for ship in ships_sorted:
                # Эмодзи в зависимости от уверенности
                if ship['confidence'] >= 0.8:
                    emoji = "✅"
                elif ship['confidence'] >= 0.6:
                    emoji = "⚠️"
                else:
                    emoji = "❓"
                
                st.markdown(f"""
{emoji} **🚢 Корабль #{ship['id']}**
...
# ============================================
    ...
    # ============ ДОБАВЛЕН ПОРОГ УВЕРЕННОСТИ ДЛЯ ТРЕКОВ ============
    st.sidebar.subheader("🎯 Порог детекции")
    
    track_conf_threshold = st.sidebar.slider(
        "Порог яркости/уверенности",
        min_value=0.1,
        max_value=0.95,
        value=0.50,  # ПО УМОЛЧАНИЮ 0.5
        step=0.05,
        format="%.2f",
        help="Минимальный порог для обнаружения треков на SAR изображении"
    )
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Запустить детекцию треков"):
            try:
                ...
                if len(tracks) > 0:
                    st.success(f"✅ Обнаружено треков: {len(tracks)} (порог: {track_conf_threshold:.0%})")
                    ...
                    st.caption(f"💡 Обнаружено {len(tracks)} треков (порог: {track_conf_threshold:.0%}). Наведите курсор для просмотра координат.")
...
{f"- Объединён из треков: {track.merged_from}" if len(track.merged_from) > 0 else ""}
                    ...
                    json_data = json.dumps({
                        ...
                        'confidence_threshold': track_conf_threshold,
                        'crs': str(crs),
                        'bounds': {
                            'left': bounds.left,
